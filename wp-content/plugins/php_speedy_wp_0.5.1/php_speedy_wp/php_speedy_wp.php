...
0.5 - Updated to work with WP 2.7
	- Added Data URIs for CSS background images
	- Removed Wordpress only gzipping
0.5.1 - Added 304 Not modified header (thanks Matt)
	  - Changed gzip code (thanks http://rakaz.nl)
*/
...
class wp_scripts_extend extends WP_Scripts {
	...
	function __construct() {
		...
		if(method_exists($WP_Scripts,'__construct')) {
	
			global $wp_query;
			if(!empty($wp_query->is_admin)) {
	...
	/**
	 * Returns script item instead of printing. WP 2.7
	 *
	**/
	function return_item( $handle ) {
	...
	global $wp_scripts;
		...
		if(method_exists($wp_scripts,'__construct')) { //WP 2.7

			$scripts = false === $handles ? $wp_scripts->queue : (array) $handles;
			$wp_scripts->all_deps( $handles );		
			$to_print = $wp_scripts->to_do;	
						
			if(is_array($to_print)) {
				foreach($to_print AS $handle) {
					$this->printed[] = 	$this->return_item($handle);
				}
			}
			...
			$to_print = apply_filters( 'print_scripts_array', array_keys($wp_scripts->to_print) );
			
			foreach( $to_print as $handle ) {
				if ( !in_array($handle, $wp_scripts->printed) && isset($wp_scripts->scripts[$handle]) ) {
					if ( $wp_scripts->scripts[$handle]->src ) { // Else it defines a group.
						$ver = $wp_scripts->scripts[$handle]->ver ? $wp_scripts->scripts[$handle]->ver : $wp_db_version;
						if ( isset($wp_scripts->args[$handle]) )
							$ver .= '&amp;' . $wp_scripts->args[$handle];
						$src = 0 === strpos($wp_scripts->scripts[$handle]->src, 'http://') ? $wp_scripts->scripts[$handle]->src : get_option( 'siteurl' ) . $wp_scripts->scripts[$handle]->src;
						$src = $wp_scripts->scripts[$handle]->src;
		...
		$this->to_print = array();
		return array_filter($this->printed);
...
class php_speedy_wp_controller {
	...
	if(!empty($this->admin->compress_options['js_libraries'])) {
		...
		$to_enqueue = explode(",",$this->admin->compress_options['js_libraries']);
			...
			$wp_scripts->enqueue($script);
			
				//Remove src
				$src = end(explode("/",$wp_scripts->scripts[$script]->src)); //WP 2.5 
				$src = $src ? $src : end(explode("/",$wp_scripts->registered[$script]->src)); //WP 2.7				
			}
			
		$wp_scripts_extend = new wp_scripts_extend();
		$scripts = $wp_scripts_extend->return_scripts($wp_scripts->queue);
		
		//Make sure we don't add the queued scripts twice
		//By excluding their file name from the compressor (the WP Enqued scripts are immune)		
		if(is_array($wp_scripts_extend->_done_queued)) {
			foreach($wp_scripts_extend->_done_queued AS $script) {
			$src = end(explode("/",$script)); 
...
class php_speedy_wp_admin {
	...
	$this->options = array('cache_dir'=>array('title'=>'Cache Directory',
			   ...
			   'ignore_list'=>array('title'=>'Ignore list',
					 						  'intro'=>'PHP Speedy can ignore certain scripts of your choosing. Please enter the filenames of the scripts or CSS files you would like to ignore below, separated by a comma.',
										      'key'=>'Ignore_List',
		   	      ...
		   	      'far_future_expires'=>array('title'=>'Far Future Expires Options',
			       			  ...
			       			  'data_uris'=>array('title'=>'Data URIs',
										      ...
										      'value'=>$compress_options['data_uris']
											  ),											  
	...
	Wordpress no longer does Gzipping
	*/
